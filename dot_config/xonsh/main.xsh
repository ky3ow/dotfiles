...
def _next_big_word_end(text: str, pos: int) -> int:
...
def _prev_big_word_start(text: str, pos: int) -> int:
    result = pos
    for start, end in _tokenize_big_words(text):
        if start < pos <= end:
            result = start
...
def custom_keybindings(bindings, **_):
    ...
    def _(event):
        ...
        buf.cursor_position = _next_big_word_end(buf.document.text, buf.cursor_position)

    @bindings.add("escape", "c-b")
    def _(event):
        buf = event.current_buffer
        buf.cursor_position = _prev_big_word_start(buf.document.text, buf.cursor_position)
